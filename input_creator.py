...
class input_gen(object):
    ...
    def __init__(self, data_path_list, sequence_length, sensor_drop, piecewise_lin_ref=125,
                 preproc=True, visualize=False, test=False):
        '''
    ...
    def concat_vec(self):
        ...
        train_vec_samples = self.seq_array_train.reshape(self.seq_array_train.shape[0],
                                                  self.seq_array_train.shape[1] * self.seq_array_train.shape[2])
        test_vec_samples = self.seq_array_test_last.reshape(self.seq_array_test_last.shape[0],
                                                self.seq_array_test_last.shape[1] * self.seq_array_test_last.shape[2])

        label_array_train = self.label_array_train
        label_array_test = self.label_array_test

        return train_vec_samples, label_array_train, test_vec_samples, label_array_test
